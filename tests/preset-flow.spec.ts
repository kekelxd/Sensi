...
test.describe('shared presets and game identity', () => {
  ...
  test('creates a profile preset and reuses it across training, calibration and conversion', async ({ page, isMobile }) => {
    ...
    await dialog.getByRole('button', { name: /Counter-Strike 2/ }).click()
    await expect(dialog.getByText('Preset principal', { exact: true })).toBeVisible()
    await dialog.getByRole('button', { name: /Continuar/ }).click()
    await expect(dialog.getByRole('textbox', { name: /Sensibilidade/ })).toHaveValue('0.65')
    await expect(dialog.getByRole('textbox', { name: 'DPI do mouse' })).toHaveValue('800')
    await dialog.getByRole('textbox', { name: /Sensibilidade/ }).fill('0.70')
    await dialog.getByRole('button', { name: /Continuar/ }).click()
    if (!isMobile) {
  ...
  test('renders a compact selector, explicit presets and stable manual edits', async ({ page }, info) => {
    ...
    await expect(dialog.locator('.xensi-preset-context small')).toContainText('0.7 · 800 DPI')
    ...
    await expect(dialog.locator('.xensi-preset-context')).toContainText('Sem preset salvo')
    await expect(dialog.getByRole('button', { name: /Continuar/ })).toBeEnabled()
    await cs.click()
    await dialog.getByRole('button', { name: /Continuar/ }).click()
    await dialog.getByRole('textbox', { name: /Sensibilidade/ }).fill('0.73')
  ...
  test('keeps anonymous training available and respects reduced motion', async ({ page }) => {
    ...
    await dialog.getByRole('button', { name: /Continuar/ }).click()
    await dialog.getByRole('textbox', { name: /Sensibilidade/ }).fill('0.25')
